...
import '../../lib/application/usecases/sms/filter_sms.dart';
import '../../lib/application/usecases/sms/log_sms.dart';
import '../../lib/application/usecases/contact/get_contacts.dart';
import '../../lib/application/usecases/contact/sync_contacts.dart';
...
import '../infrastructure/services/caller/caller_id_service_impl.dart';
import '../infrastructure/services/caller/caller_id_monitor_service_impl.dart';
import '../infrastructure/services/notification_service_impl.dart';
import '../application/usecases/call/handle_incoming_call.dart';
import '../application/usecases/call/should_accept_call.dart';
...
Future<void> registerCallerIdDependencies() async {
  // 平台通道
  getIt.registerSingleton<CallChannelManager>(CallChannelManager());
  
  // 服务实现
  getIt.registerSingleton<CallerIdService>(
    CallerIdServiceImpl(
  ...
  getIt.registerSingleton<CallerIdMonitorService>(
    CallerIdMonitorServiceImpl(
      getIt<CallerIdService>(),
      getIt<CallChannelManager>(),
      getIt<NotificationServiceImpl>(),
      await SharedPreferences.getInstance(),
    ),
  );
  
  // 用例
  getIt.registerFactory<HandleIncomingCallUseCase>(
    () => HandleIncomingCallUseCase(getIt<CallerIdMonitorService>()),
  );
  
  getIt.registerFactory<ShouldAcceptCallUseCase>(
    () => ShouldAcceptCallUseCase(getIt<CallerIdMonitorService>()),
  );
}