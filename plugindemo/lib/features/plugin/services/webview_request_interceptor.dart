...
class WebViewRequestInterceptor {
  ...
  Future<WebResourceResponse?> _handleLeakedRequest(
    ...
    try {
      ...
      _addLog(
        'Making backend HTTP GET for LEAKED request to: $uri with headers: ${request.headers}',
      );
      final response = await http.get(uri, headers: request.headers);
      _addLog(
        'Backend response for LEAKED request: ${response.statusCode} for $uri',
      );
    ...
    try {
      ...
      final processedHtml = htmlBody.replaceAllMapped(attrRegex, (match) {
        ...
        try {
          ...
          final newProxyUrl =
              Uri.https(PROXY_HOST, PROXY_PATH_FETCH, {
                'requestId': requestId ?? '',
