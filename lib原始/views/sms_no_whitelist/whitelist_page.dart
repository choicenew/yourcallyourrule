...
class SmsWhitelistPageView extends StatelessWidget {
  const SmsWhitelistPageView({super.key});
  ...
  Widget build(BuildContext context) {
    return buildPageWithCollapsibleContent(
     ...
     // 'SmsWhitelistPageView', // 卡片标题=点击卡片导航到的页面
      ...
      const SmsWhitelistPage(), // 当前页面主要内容
...
class SmsWhitelistPage extends StatefulWidget {
  const SmsWhitelistPage({super.key});

  @override
  SmsWhitelistPageState createState() => SmsWhitelistPageState();
}

class SmsWhitelistPageState extends State<SmsWhitelistPage>
    with WidgetsBindingObserver {
  // final SmsWhitelistService _subscriptionService =
  //     SmsWhitelistService(database as Database);
  ...
  List<SmsWhitelistEntry> _entries = [];
  ...
  Future<void> _loadEntries() async {
    ...
    try {
      final entries = await _smsWhitelistService.getEntries(orderBy: 'phoneNumber', ascending: true); // 这里传入 phoneNumber 作为 orderBy 参数
      setState(() {
        _entries = entries;
        _isLoading = false;
      });
    ...
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: _isLoading
            ...
            : AdvancedSelfManagedSearchBar<SmsWhitelistEntry>(
                items: _entries,
...
// 私有的构建订阅列表方法
  ...
  Widget _buildEntryItem(SmsWhitelistEntry entry, double entryWidth) {
    return Card(
  ...
  Widget _buildExpandedContent(BuildContext context, SmsWhitelistEntry entry) {
...
/*
// SmsWhitelistPage
  void _showDeleteConfirmation(BuildContext context, SmsWhitelistEntry entry) {
    showDeleteConfirmationDialog(
      ...
      phoneNumber: entry.phoneNumber, // 传递 phoneNumber 参数 代表删除的时候展示的关键词
      // keyword: entry.keyword, // 传递 keyword 参数
    );
  }
*/
// SmsWhitelistPage
void _showDeleteConfirmation(BuildContext context, SmsWhitelistEntry entry) {
  showDeleteConfirmationDialog<SmsWhitelistEntry>(
    context,
    entry,
    _smsWhitelistService.remove,
  ...
  void _saveEntry(SmsWhitelistEntry entry, String name, String avatar,
      bool isWhitelist, bool isBlacklist) async {
    // 添加 isAllowed 和 isBlocked 参数
    final updatedEntry = SmsWhitelistEntry(
      phoneNumber: entry.phoneNumber,
      label: entry.label,
      name: name,
      avatar: avatar,
    ...
    if (isWhitelist) {
      // 使用传递的 isWhitelist
 await _smsWhitelistService.addOrUpdate(updatedEntry);
