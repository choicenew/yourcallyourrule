...
class WhitelistPageView extends StatelessWidget {
  const WhitelistPageView({super.key});
...
class WhitelistPage extends StatefulWidget {
  ...
  @override
  WhitelistPageState createState() => WhitelistPageState();
}

class WhitelistPageState extends State<WhitelistPage>
    with WidgetsBindingObserver {
  // final WhitelistService _subscriptionService =
  //     WhitelistService(database as Database);
  ...
  Future<void> _loadEntries() async {
    ...
    try {
      final entries = await _whitelistService.getEntries(orderBy: 'phoneNumber', ascending: true); // 这里传入 phoneNumber 作为 orderBy 参数
      setState(() {
        _entries = entries;
        _isLoading = false;
      });
    ...
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: _isLoading
            ...
            : AdvancedSelfManagedSearchBar<WhitelistEntry>(
                items: _entries,
...
// 私有的构建订阅列表方法
  ...
  Widget _buildEntryItem(WhitelistEntry entry, double entryWidth) {
    return Card(
  ...
  Widget _buildExpandedContent(BuildContext context, WhitelistEntry entry) {
...
// WhitelistPage
void _showDeleteConfirmation(BuildContext context, WhitelistEntry entry) {
  showDeleteConfirmationDialog<WhitelistEntry>(
    context,
    entry,
    _whitelistService.remove,
  ...
  void _saveEntry(WhitelistEntry entry, String name, String avatar,
      bool isAllowed, bool isBlocked) async {
    // 添加 isAllowed 和 isBlocked 参数
    final updatedEntry = WhitelistEntry(
      phoneNumber: entry.phoneNumber,
      label: entry.label,
      name: name,
      avatar: avatar,
    ...
    await _whitelistService.addOrUpdate(updatedEntry);
    if (isAllowed) {
      // 使用传递的 isAllowed
