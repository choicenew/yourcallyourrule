...
class CallFilter {
  ...
  Future<void> loadConfig() async {
    ...
    config = CallFilterConfig(
      ...
      allowAllAllowedNumbers:
          await asyncPrefs.getBool('allowAllAllowedNumbers') ?? true,
      allowRegexWhitelistedPatterns:
          await asyncPrefs.getBool('allowRegexWhitelistedPatterns') ?? true,
      allowBlockedNumbers:
          await asyncPrefs.getBool('allowBlockedNumbers') ?? false,
      allowAllWhitelistedNumbers:
          await asyncPrefs.getBool('allowAllWhitelistedNumbers') ?? true,
      allowRegexBlacklistedPatterns:
          await asyncPrefs.getBool('allowRegexBlacklistedPatterns') ?? true,
      allowAllBlacklistedNumbers:
          await asyncPrefs.getBool('allowAllBlacklistedNumbers') ?? false,
    );
  }
  ...
  Future<void> saveConfig() async {
    ...
    await asyncPrefs.setBool(
        'allowAllAllowedNumbers', config.allowAllAllowedNumbers);
    await asyncPrefs.setBool(
        'allowRegexWhitelistedPatterns', config.allowRegexWhitelistedPatterns);
    await asyncPrefs.setBool('allowBlockedNumbers', config.allowBlockedNumbers);
    await asyncPrefs.setBool(
        'allowAllWhitelistedNumbers', config.allowAllWhitelistedNumbers);
    await asyncPrefs.setBool(
        'allowRegexBlacklistedPatterns', config.allowRegexBlacklistedPatterns);
    await asyncPrefs.setBool(
        'allowAllBlacklistedNumbers', config.allowAllBlacklistedNumbers);
  }
  ...
  static Future<CallFilter> create(
      {required Database database, CallFilterConfig? config}) async {
    return CallFilter(
  ...
  Future<bool> shouldAcceptCall(String phoneNumber) async {
    ...
    if (config.allowRegexWhitelistedPatterns) {
      final whitelistedPatterns = await regexService.getWhitelistedPatterns();
...
// 检查号码是否匹配黑名单通配符
    if (config.allowRegexBlacklistedPatterns) {
      final blacklistedPatterns = await regexService.getBlacklistedPatterns();
