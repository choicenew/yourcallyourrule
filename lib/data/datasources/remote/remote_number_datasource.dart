...
class RemoteNumberDataSource
  ...
  Future<RemoteNumberModel?> getById(String phoneNumber) async {
    final result =
        await (_database.select(_database.remoteNumbers)
  ...
  Future<String> insert(
    RemoteNumberModel remoteNumber, {
    ...
    await _database.transaction(() async {
      await _database
          ...
          .insert(
            _toRemoteNumberCompanion(
              remoteNumber,
            ).copyWith(id: Value(_uuid.v4())),
            mode: InsertMode.insertOrReplace,
          );

      if (countryIsoCode != null) {
        await _database
            .into(_database.numberCountries)
            .insert(
      ...
      final payload = remoteNumber.toMap();
      if (countryIsoCode != null) {
  ...
  Future<int> update(
    ...
    int result = 0;
    await _database.transaction(() async {
      result = await (_database.update(_database.remoteNumbers)..where(
        (tbl) => tbl.phoneNumber.equals(remoteNumber.phoneNumber),
    ...
    await _database.transaction(() async {
      result =
          await (_database.delete(_database.remoteNumbers)
            ..where((tbl) => tbl.phoneNumber.equals(phoneNumber))).go();
    });
    return result;
  }
  ...
  Future<int> atomicIncrementCount(
    ...
    return await _database.transaction(() async {
      final result = await (_database.update(_database.remoteNumbers)
        ..where((tbl) => tbl.phoneNumber.equals(phoneNumber))).write(
        // 【正确方案】使用 .custom 构造函数来处理 Expression
  ...
  Future<List<Map<String, dynamic>>> getLocalPendingOperations() async {
    final results =
        await (_database.select(_database.pendingOperations)..orderBy([
          (tbl) =>
    ...
    await _database.transaction(() async {
      for (final change in changes) {
        ...
        switch (operation) {
          ...
          case 'DELETE':
            await (_database.delete(_database.remoteNumbers)
              ..where((tbl) => tbl.phoneNumber.equals(entityId))).go();
            break;
          ...
          case 'DELETION_PROPOSAL_INFO':
            ...
            final companion = ActiveDeletionProposalsCompanion(
              ...
              proposalStartTime: Value(
                payload['proposalStartTime']?.toString() ??
                    DateTime.now().toIso8601String(),
              ),
              status: const Value('pending'), // 从服务器拉取的活跃提议总是 pending
              highestRiskLevel: Value(
                payload['highestRiskLevel']?.toString() ?? 'Unknown',
              ),
              proposalCount: Value(
                (payload['proposalCount'] as num?)?.toInt() ?? 0,
              ),
              verifiedOwnerCount: Value(
                (payload['verifiedOwnerCount'] as num?)?.toInt() ?? 0,
              ),
              lastUpdated: Value(DateTime.now().toIso8601String()), // 本地更新时间
              verificationReportJson: Value(
                payload['verificationReportJson']?.toString(),
              ),
            );

            // 执行插入或替换
            await _database
                .into(_database.activeDeletionProposals)
          ...
          case 'INCREMENT':
            if (payload != null && payload.containsKey('increment')) {
              ...
              await (_database.update(_database.remoteNumbers)
                ..where((tbl) => tbl.phoneNumber.equals(entityId))).write(
                RemoteNumbersCompanion.custom(
                  count:
                      _database.remoteNumbers.count + Constant(incrementValue),
                ),
              );
            }
  ...
  Future<String> insertSyncRecord(SyncRecordModel syncRecord) async {
    await _database
        ...
        .insert(_toSyncRecordCompanion(syncRecord));
  ...
  Future<bool> atomicVote(String phoneNumber, String label) async {
    ...
    await _database.transaction(() async {
      ...
      if (existingNumber != null) {
        ...
        result = await (_database.update(_database.remoteNumbers)..where(
          (tbl) => tbl.phoneNumber.equals(phoneNumber),
        )).write(companion);
  ...
  Future<List<RemoteNumberModel>> getByLabel(String label) async {
    final results =
        await (_database.select(_database.remoteNumbers)
          ..where((tbl) => tbl.label.equals(label))).get();
  ...
  Future<List<RemoteNumberModel>> getByPriority(int priority) async {
    final results =
        await (_database.select(_database.remoteNumbers)
          ..where((tbl) => tbl.priority.equals(priority))).get();
  ...
  Future<List<RemoteNumberModel>> getByAction(String action) async {
    final results =
        await (_database.select(_database.remoteNumbers)
          ..where((tbl) => tbl.action.equals(action))).get();
  ...
  Future<int> deleteNumbersByCountry(String countryIsoCode) async {
    ...
    await _database.transaction(() async {
      final numbersToDelete =
          await (_database.select(_database.numberCountries)
            ..where((tbl) => tbl.countryIsoCode.equals(countryIsoCode))).get();
      ...
      await (_database.delete(_database.numberCountries)
        ..where((tbl) => tbl.countryIsoCode.equals(countryIsoCode))).go();
      ...
      if (orphanedNumbers.isNotEmpty) {
        final result =
            await (_database.delete(_database.remoteNumbers)
              ..where((tbl) => tbl.phoneNumber.isIn(orphanedNumbers))).go();
  ...
  Future<void> bulkInsertFromServer(
    ...
    await _database.batch((batch) {
      for (final numberMap in numbers) {
        ...
        final numberCompanion = RemoteNumberData.fromJson(
          numberMap,
        ).toCompanion(true);
        batch.insert(
