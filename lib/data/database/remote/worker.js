...
async function handlePush(request, env) {
  ...
  for (const op of operations) {
    if (typeof op.payload === 'string') {
      try {
        // 尝试拆箱
        parsedOperations.push({ ...op, payload: JSON.parse(op.payload) });
      } catch (e) {
        // 🚨 核心修改在这里：如果报错，什么都不做，直接抛弃这条数据！
        console.warn(`[拦截] 发现无法解析的脏数据，已丢弃: ${op.payload}`);
      }
    } else {
      // 本来就是对象的正常数据，直接放行
  ...
  const regularOps = parsedOperations.filter(op => op.operation !== 'PROPOSE_DELETION');
  const proposalOps = parsedOperations.filter(op => op.operation === 'PROPOSE_DELETION');
  ...
  for (const proposal of activeProposals) {
    ...
    finalOperations.push({
      ...
      payload: {
        ...
        proposalStartTime: proposal.proposal_start_time
      }
    });
  }
      ...
      switch (op.operation) {
        ...
        case 'UPDATE':
          if (payload) {
            ...
            // [NEW] Handle country code insertion
            if (payload.countryIsoCode) {
              finalStmts.push(
                env.DB.prepare(
                  'INSERT INTO number_countries (phone_number, country_iso_code) VALUES (?, ?) ON CONFLICT(phone_number, country_iso_code) DO NOTHING'
