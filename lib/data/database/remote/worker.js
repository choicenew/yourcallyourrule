    ...
    if (url.pathname === '/sync') {
      ...
      try {
        if (request.method === 'GET') {
          // 如果是 GET 请求，说明客户端想要获取特定国家的数据。
          return await handleCountryData(request, env);
        } else {
          // 如果是其他方法，我们不支持，返回错误。
          return errorResponse('Method Not Allowed', 405);
        }
      } catch (e) {
        console.error(`Error during country data fetch: ${e.stack}`);
        return errorResponse(e.message, 500);
      }
    }

    // 如果请求的路径不是 `/sync` 或 `/country-data`，说明客户端访问了一个不存在的地址。
    return errorResponse('Not Found', 404);
  },
};

// =============================================================================
// SECTION: COUNTRY DATA HANDLER (GET /country-data)
// 这个函数处理客户端获取特定国家数据的请求
// =============================================================================
async function handleCountryData(request, env) {
  ...
  if (!countryCode) {
    return errorResponse('Missing countryCode parameter', 400);
  }
  
  try {
    // 查询数据库获取特定国家代码的号码数据
    const { results } = await env.DB.prepare(`
      SELECT * FROM remote_numbers 
    ...
    console.error(`Error fetching country data: ${error.stack}`);
    return errorResponse(`Failed to fetch country data: ${error.message}`, 500);
  }
}
