    ...
    switch (op.operation) {
      ...
      case 'VOTE':
        ...
        const currentRecord = await env.DB.prepare(
          `SELECT count, label FROM remote_numbers WHERE phoneNumber = ?`
        ).bind(phoneNumber).first();
        
        if (currentRecord) {
          const increment = payload.increment || 1;
          const newCount = (currentRecord.count || 0) + increment;
          const updatedLabel = payload.label || currentRecord.label;
          
          // 操作 3b: 更新计数和标签
          stmts.push(
            env.DB.prepare(
              `UPDATE remote_numbers 
               SET count = ?, label = ? 
               WHERE phoneNumber = ?`
            ).bind(newCount, updatedLabel, phoneNumber)
          );
        }
        break;
