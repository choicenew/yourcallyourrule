...
class ImportSmsTextBlacklistsPageView extends StatelessWidget {
  const ImportSmsTextBlacklistsPageView({super.key});

  @override
  Widget build(BuildContext context) {
    return buildPageWithCollapsibleContent(
      context,
      S.of(context).importSmsTextBlacklistsPage, // 当前页面标题
      'SmsTextBlacklistPageView', // 卡片标题=点击卡片导航到的页面
      const SmsTextBlacklistPageView(), // 点击卡片导航到的页面
      const ImportSmsTextBlacklistsPage(), // 当前页面主要内容
      //下面分别是导出，添加，导入
...
class ImportSmsTextBlacklistsPage extends StatefulWidget {
  const ImportSmsTextBlacklistsPage({super.key});

  @override
  ImportSmsTextBlacklistsPageState createState() =>
      ImportSmsTextBlacklistsPageState();
}

class ImportSmsTextBlacklistsPageState
    extends State<ImportSmsTextBlacklistsPage> {
          ...
          child: ElevatedButton(
            onPressed: () {
              ...
              if (_urlController.text.isNotEmpty) {
                _smsTextBlacklistService
                    .importKeywordsFromUrl(_urlController.text);
              } else if (filePath != null) {
                _smsTextBlacklistService.importKeywordsFromLocal(filePath!);
              } else {
                showErrorSnackBar(
