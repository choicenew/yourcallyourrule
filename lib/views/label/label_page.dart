...
class LabelPageState extends State<LabelPage> with WidgetsBindingObserver {
  // final LabelService _subscriptionService =
  //     LabelService(database as Database);
  ...
  Future<void> _loadEntries() async {
    ...
    try {
      final entries = await _labelService.getEntries(
          orderBy: 'phoneNumber',
    ...
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: _isLoading
            ...
            : AdvancedSelfManagedSearchBar<LabeledEntry>(
                items: _entries,
...
// 私有的构建订阅列表方法
  ...
  Widget _buildEntryItem(LabeledEntry entry, double entryWidth) {
    return Card(
  ...
  Widget _buildExpandedContent(BuildContext context, LabeledEntry entry) {
...
// LabelPage
  void _showDeleteConfirmation(BuildContext context, LabeledEntry entry) {
    showDeleteConfirmationDialog<LabeledEntry>(
      context,
      entry,
      _labelService.remove,
      displayText: entry.phoneNumber, // 传递 phoneNumber 参数 代表删除的时候展示的关键词
    );
  }

  void _saveEntry(LabeledEntry entry, String name, String avatar,
    ...
    await _labelService.addOrUpdate(updatedEntry);
    if (isAllowed) {
      // 使用传递的 isAllowed
