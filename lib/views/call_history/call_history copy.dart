...
Future<void> _loadPreferences() async {
 ...
 //获取通话记录
  ...
  setState(() {
    ...
    PermissionStatus status = await Permission.phone.status; // 使用 Permission.phone.status 检查权限状态
    if (status.isGranted) {
...
Future<CallerIdData> getCallerIdData(CallLogEntry entry, BuildContext context) async {
  // 1. 初始化 sim_card_info 插件
  final simCardInfoPlugin = SimCardInfo();

  String? phoneNumber = entry.number ?? '';
  print(' 来电号码: $phoneNumber');
  // 2. 获取 SIM 卡信息
    ...
    try {
      PhoneNumber parsedPhoneNumber =
          phoneNumberUtil.parse(phoneNumber, null);
           print('解析后的号码1：$parsedPhoneNumber'); // 打印 parsedCountryCode
      countryCode =
      ...
      try {
        PhoneNumber parsedPhoneNumber =
            phoneNumberUtil.parse(phoneNumber, simCountryCode);
             print('解析后的sim国家代码：$simCountryCode'); // 打印 parsedCountryCode
print('解析后的号码2：$parsedPhoneNumber'); // 打印 parsedCountryCode
        // 验证解析结果
...
try {
 ...
 int? numericCountryCode = phoneNumberUtil.getCountryCodeForRegion(simCountryCode.toUpperCase());
    print('解析后的numeric国家代,$numericCountryCode'); 
  ...
  print('解析后的sim国家代码,$simCountryCode'); 
  print('解析后的号码3,$fullPhoneNumber'); 
  print('解析后的号码3,$parsedPhoneNumber'); 
  // 验证解析结果
  String? parsedCountryCode =
      phoneNumberUtil.getRegionCodeForNumber(parsedPhoneNumber);
  if (parsedCountryCode?.toUpperCase() ==
  ...
  print('Failed to parse with country code (with +) $simCountryCode: $e'); // 修改错误信息
}  
...
print(dlibLocale.runtimeType);
  print('language code $languageCode'); 
              print('dlibLocale.language: ${dlibLocale.language}');
              print('dlibLocale.country: ${dlibLocale.country}');
...
if (phoneNumber.isEmpty) {
  ...
  CallerIdData callerIdData = await _callerIdService.getCallerId(
      e164Number, context, dlibLocale);
         print('try 里面的dlibcountry ${dlibLocale}');
   print('dlibcountry code ${dlibLocale.country}');
  print('dliblanguage code  ${dlibLocale.language}');    
  return callerIdData;
} catch (error) {
  print('call history 的Error getting caller ID data: $error');
           print('catch 里面的dlibcountry $dlibLocale');
   print('catch dlibcountry code ${dlibLocale.country}');
  print(' catch dliblanguage code  ${dlibLocale.language}'); 
      return CallerIdData(
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () => Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => const AddLabelPage()),
        ),
      ),
    );
  }
...
return ListView(
  children: [
    Padding(
      ...
      child: Wrap(
        ...
        children: List.generate(_callLogEntry.length + (adState.isAdEnabled ? _callLogEntry.length ~/ 3 : 0), (index) {
          if ((index + 1) % 4 == 0 && adState.isAdEnabled) {
           return nativeAdWidgetSmall(adWidth: entryWidth, adHeight: 90);
            ...
            return SizedBox(
              width: entryWidth,
                      child: _buildCallLogItem(_callLogEntry[adjustedIndex], entryWidth),
                    );
                  }
                }),
              ),
 ...
 Widget _buildCallLogItem(CallLogEntry entry, double entryWidth) {
  return Card(
    ...
    child: FutureBuilder<CallerIdData>(
     future: getCallerIdData(entry, context), // 使用 getCallerIdData 获取数据
...
*/
        ...
        onPressed: () => Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) => ContactFormPage(),
              ...
              // 展开/折叠图标
            Icon(
              _expandedItems[entry.number] == true 
                  ? Icons.expand_less 
                  : Icons.expand_more
            ),
            ],
  ...
  Widget _buildExpandedContent(
      BuildContext context, CallerIdData callerIdData) {
        final TextEditingController nameController = TextEditingController(text: callerIdData.name);
    //新增加的avatar 路径
    final TextEditingController avatarController = TextEditingController(text: callerIdData.avatar);
            ...
            children: [
              ...
              ElevatedButton(
                ...
                final updatedEntry = LabeledEntry(
                    ...
                    avatar: avatarController.text, // Use the avatarController's value                    
...
class _LabelsState extends State<Labels> {
  ...
  Widget build(BuildContext context) {
    ...
    fontSize: 16.0,
    backgroundColor: Color.fromRGBO(147, 203, 128, 1), // Add background color here
    fontWeight: FontWeight.bold,
  ),
            ),
          ),
