...
class ContactSubscriptionListView extends StatelessWidget {
  const ContactSubscriptionListView({super.key});
...
class ContactSubscriptionList extends StatefulWidget {
  const ContactSubscriptionList({super.key});

  @override
  ContactSubscriptionListState createState() => ContactSubscriptionListState();
}

class ContactSubscriptionListState extends State<ContactSubscriptionList> with WidgetsBindingObserver {

  late SubscribeContactsService _subscribeContactsService;
  Map<int, bool> _expandedItems = {};
  List<ContactSubscriptionModel> _subscriptions = [];
  ...
  Future<void> _loadSubscriptions() async {
    ...
    try {
      final subscriptions = await _subscribeContactsService.getAllSubscriptions(orderBy: 'id', ascending: true);
      setState(() {
  ...
  return Scaffold(
    body: RefreshIndicator(
      ...
      child: _isLoading
          ...
          : AdvancedSelfManagedSearchBar<ContactSubscriptionModel>(
...
// 私有的构建订阅列表方法
  ...
  Widget _buildSubscriptionItem(
      ContactSubscriptionModel subscription, double subscriptionWidth) {
    return Card(
  ...
  Widget _buildExpandedContent(
      BuildContext context, ContactSubscriptionModel subscription) {
        ...
        children: [
          ...
          SubscriptionAutoUpdateSwitchWidget(
            ...
            onChanged: (bool newValue) async {
              ...
              if (newValue) {
                await _subscribeContactsService.autoUpdateSubscription(subscription);
              }
              setState(() {});
            },
          ),
  ...
  void _showDeleteConfirmation(
      BuildContext context, ContactSubscriptionModel subscription) {
    showDeleteConfirmationDialog<ContactSubscriptionModel>(
      context,
      subscription,
      _subscribeContactsService.deleteSubscription,
      displayText: subscription.id.toString(),
    );
  }

// 在 ContactSubscriptionListState 中定义一个新的方法

Future<void> _toggleSubscriptionEnabled(ContactSubscriptionModel subscription) async {
  subscription.enabled = !subscription.enabled;

  await _subscribeContactsService.editSubscription(subscription);

  if (subscription.enabled) {
        await _subscribeContactsService.enableSubscription(subscription);
  } else {
    await _subscribeContactsService.disableSubscription(subscription);
  }
...
Future<void> _updateSubscription(ContactSubscriptionModel subscription, String name, String url, bool isWhitelist, bool isBlacklist) async {
  final updatedSubscription = ContactSubscriptionModel(
    id: subscription.id,
    name: name,
    url: url,
  ...
  await _subscribeContactsService.editSubscription(updatedSubscription);
await _contactService.importContactsFromUrl(subscription.url);

}

void _saveEntry(ContactSubscriptionModel subscription, String name, String url, bool isWhitelist, bool isBlacklist) async {
