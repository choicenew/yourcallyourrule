...
class AllowedLabels extends StatefulWidget {
  ...
  @override
  State<AllowedLabels> createState() => _AllowedLabelsState();
}

class _AllowedLabelsState extends State<AllowedLabels> {
  ...
  Future<void> _initLabels() async {
    // 获取所有标签
    final labels = await _allowedService.getAllLabels();
    if (labels.isEmpty) {
      await _allowedService
          .insertPredefinedLabelsIfNotExists(_allowedService.database);
      // 重新获取标签列表
      _labels = await _allowedService.getAllLabels();
    } else {
      _labels = labels;
    }
    setState(() {});
