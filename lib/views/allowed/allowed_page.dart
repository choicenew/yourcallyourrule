...
class AllowedPageView extends StatelessWidget {
  const AllowedPageView({super.key});
...
class AllowedPage extends StatefulWidget {
  const AllowedPage({super.key});

  @override
  AllowedPageState createState() => AllowedPageState();
}

class AllowedPageState extends State<AllowedPage> with WidgetsBindingObserver {
  ...
  Future<void> _loadEntries() async {
    ...
    try {
      final entries = await _allowedService.getEntries();
      setState(() {
        _entries = entries;
        _isLoading = false;
      });
    ...
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: _isLoading
            ...
            : AdvancedSelfManagedSearchBar<AllowedEntry>(
                items: _entries,
...
// 私有的构建订阅列表方法
  ...
  Widget _buildEntryItem(AllowedEntry entry, double entryWidth) {
    return Card(
  ...
  Widget _buildExpandedContent(BuildContext context, AllowedEntry entry) {
...
// AllowedPage
  void _showDeleteConfirmation(BuildContext context, AllowedEntry entry) {
    showDeleteConfirmationDialog<AllowedEntry>(
      context,
      entry,
      _allowedService.remove,
      displayText: entry.phoneNumber, // 传递 phoneNumber 参数 代表删除的时候展示的关键词
    );
  }

  void _saveEntry(AllowedEntry entry, String name, String avatar,
      bool isAllowed, bool isBlocked) async {
    // 添加 isAllowed 和 isBlocked 参数
    final updatedEntry = AllowedEntry(
      phoneNumber: entry.phoneNumber,
      label: entry.label,
      name: name,
      avatar: avatar,
    ...
    await _allowedService.addOrUpdate(updatedEntry);
    if (isAllowed) {
      // 使用传递的 isAllowed
