  ...
  */
    ...
    try {
      return await completer.future.timeout(
        ...
        onTimeout: () {
          ...
          _sessionOrigins.remove(requestId); // Cleanup
  ...
  Future<Map<String, dynamic>?> generatePluginOutput(
    ...
    try {
      return await completer.future.timeout(
        ...
        onTimeout: () {
          _pluginQueryCompleters.remove(requestId);
          _sessionOrigins.remove(requestId); // 清理会话
  ...
  Future<WebResourceResponse?> handleInterceptedRequest(
    ...
    if (uri.scheme == PROXY_SCHEME &&
      ...
      return _handleOriginalProxyRequest(request);
    }

    // 2. 处理泄漏请求 - 直接访问原始域名的请求
        ...
        if (refererUri.host == PROXY_HOST) {
          ...
          if (requestId != null) {
            ...
            if (expectedOrigin != null && uri.origin == expectedOrigin) {
              _addLog(
                  '🎯 Precisely caught LEAKED request via Referer: ${uri.toString()}');
              return _handleLeakedRequest(request);
            }
          }
        }
      } catch (e) {
