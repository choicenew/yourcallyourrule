...
class PluginTestService {
  ...
  Future<WebResourceResponse?> _shouldInterceptRequest(
    ...
    if (uri.scheme == PROXY_SCHEME && uri.host == PROXY_HOST) {
      ...
      return _handleOriginalProxyRequest(
          controller, request); // 使用您原封不动的原始处理器
    }
        ...
        if (refererUri.host == PROXY_HOST) {
          ...
          if (requestId != null) {
            ...
            if (expectedOrigin != null && uri.origin == expectedOrigin) {
              _addLog(
                  '🎯 Precisely caught LEAKED request via Referer for session [$requestId]: ${request.url}');
    ...
    if (_sessionOrigins.values.contains(uri.origin)) {
      _addLog(
          '🔥 Insurance layer caught a request to an active origin: ${uri.toString()}');
      return _handleLeakedRequest(controller, request);
    }
...
// =======================================================================
  ...
  String _preprocessHtml(
      String htmlBody, http.Response response, Uri requestUri) {
    ...
    final attrRegex = RegExp(r'''(src|href)\s*=\s*("([^"]+)"|\'([^\']+)\')''',
        caseSensitive: false);
    ...
    final processedHtml = htmlBody.replaceAllMapped(attrRegex, (match) {
      ...
      try {
        final fullOriginalUrl =
            Uri.parse(targetOrigin).resolve(path).toString();
        final newProxyUrl = Uri.https(PROXY_HOST, PROXY_PATH_FETCH, {
          'requestId': requestId ?? '',
          'originalOrigin': targetOrigin,
          'targetUrl': fullOriginalUrl
        }).toString();
  ...
  Future<WebResourceResponse?> _handleOriginalProxyRequest(
    ...
    if (uri.scheme == PROXY_SCHEME &&
      ...
      if (targetUrlParam == null || targetUrlParam.isEmpty) {
        ...
        return WebResourceResponse(
          ...
          data: Uint8List.fromList(
              'Proxy Error: Missing targetUrl parameter'.codeUnits),
          statusCode: 400,
        );
      }
      ...
      try {
        ...
        if (headersParam != null && headersParam.isNotEmpty) {
          try {
            final decodedHeaders =
                jsonDecode(Uri.decodeComponent(headersParam))
                    as Map<String, dynamic>;
            decodedHeaders.forEach(
                (key, value) => requestHeaders[key] = value.toString());
        ...
        _addLog(
            'Backend response received: ${response.statusCode} for $targetUrl');
        ...
        String combinedScript =
            domainBlockerScript + networkInterceptorScript + receiverScript;
//判断contenttype
        final contentType = response.headers['content-type'] ?? '';
        ...
        if (contentType.contains('text/html')) {
          if (htmlBody.contains('<head>')) {
            ...
            htmlBody = htmlBody.replaceFirst(
                '<html>', '<html><head>$combinedScript</head>');
          ...
          _addLog(
              '⏩ Skipping script injection for non-HTML content ($contentType).');
        }
...
// 修复后的Universal Network Interceptor Script
  ...
  /// 处理器二：处理从iframe“泄漏”出来的、发往原始域的请求
  /// 这种请求通常是页面内的脚本（如XHR/fetch）发起的，它们没有经过我们重写的URL。
