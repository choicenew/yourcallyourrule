...
class PluginUrlWebViewService {
  ...
  Future<Map<String, dynamic>?> extractUrlFromScript(PluginEntry plugin) async {
    ...
    if (scriptContent.isEmpty) {
      debugPrint('[PluginUrlWebViewService] Script is empty for plugin ${plugin.id}');
      return null;
    }
  // 2. 定义正则表达式
  final baseUrlRegex = RegExp(r"""const\s+baseUrl\s*=\s*[`'"](https?:\/\/[^/]+\/)[^'"`]*[`'"];""");
  final targetUrlRegex = RegExp(r"""const\s+targetSearchUrl\s*=\s*[`'"](https?:\/\/[^/]+\/)""");
  ...
  if (baseUrlMatch != null) {
    ...
    if (targetUrlMatch != null) {
      ...
      debugPrint('[PluginService] 提取策略：未找到 baseUrl, 但成功找到 [targetSearchUrl]: $finalUrl');
    }
  }
  ...
  if (headersMatch != null) {
    try {
      ...
      final decodedHeaders = jsonDecode(headersString) as Map<String, dynamic>;
  ...
  Future<String> generateUrlFromPhoneNumber(PluginEntry plugin, String phoneNumber) async {
    debugPrint('[PluginUrlWebViewService] Starting dynamic URL generation for plugin ${plugin.id}');
    ...
    _headlessWebView = HeadlessInAppWebView(
      ...
      onWebViewCreated: (controller) async {
        ...
        controller.addJavaScriptHandler(
          ...
          callback: (args) {
            if (args.isNotEmpty && args[0] is String) {
              ...
              if (result.startsWith('error:') && !(_urlCompleter?.isCompleted ?? true)) {
                debugPrint('[PluginUrlWebViewService] JS Error: ${result.substring(6)}');
        ...
        if (scriptContent.isNotEmpty) {
          ...
          _urlCompleter?.completeError(Exception("Script not found for plugin ${plugin.id}"));
        }
      },
      shouldInterceptRequest: _shouldInterceptRequest,
      onConsoleMessage: (controller, consoleMessage) {
        debugPrint("Headless Console [${consoleMessage.messageLevel}]: ${consoleMessage.message}");
      },
    );
    ...
    try {
      final url = await _urlCompleter!.future.timeout(const Duration(seconds: 20));
      debugPrint('[PluginUrlWebViewService] Dynamic URL generation successful: $url');
      return url;
    } catch (e) {
      debugPrint("[PluginUrlWebViewService] Error generating URL dynamically: $e");
      rethrow;
    } finally {
      dispose();
    }
  ...
  Future<WebResourceResponse?> _shouldInterceptRequest(InAppWebViewController controller, WebResourceRequest request) async {
    ...
    debugPrint('[PluginUrlWebViewService] Intercepted request: ${uri.toString()}');

    if (uri.scheme == PROXY_SCHEME && uri.host == PROXY_HOST && uri.path.startsWith(PROXY_PATH_FETCH)) {
      debugPrint('[PluginUrlWebViewService] Proxy request matched: ${uri.toString()}');
      
      if (!(_urlCompleter?.isCompleted ?? true)) {
        _urlCompleter?.complete(uri.toString());
        return WebResourceResponse(contentType: "text/plain", data: Uint8List.fromList([]), statusCode: 200);
      }
    }
    return null;
  }
