...
class _AllowedBlockedPageState extends State<AllowedBlockedPage> with SingleTickerProviderStateMixin {
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                ...
                // 使用选择的规则动作
                // 如果是允许规则，强制使用allow动作
                // 如果是阻止规则，使用用户在RuleActionSelector中选择的动作
                RuleAction finalAction = isAllowed ? RuleAction.allow : selectedAction;
                
                // 添加规则
                await service.addAllowedBlockedRule(entry, finalAction);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          // 显示当前筛选条件
          if (_selectedLabel != null || _selectedActionType != null)
            Padding(
  ...
  Widget _buildRulesList(List<AllowedBlockedRule> rules, bool isAllowed) {
    ...
    // 标签筛选
    if (_selectedLabel != null) {
      filteredRules = filteredRules.where((rule) => rule.labelId == _selectedLabel).toList();
    }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              (_selectedLabel != null || _selectedActionType != null)
