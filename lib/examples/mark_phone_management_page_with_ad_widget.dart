...
import 'package:yourcallyourrule/ads/ad_list_widget.dart'; // 导入新的AdListWidget
...
/// 号码标记管理页面 - 使用AdListWidget的示例
/// 用于用户添加号码标记，查看标记次数，以及兑换VIP功能
class MarkPhoneManagementPageWithAdWidget extends ConsumerStatefulWidget {
  const MarkPhoneManagementPageWithAdWidget({super.key});

  @override
  ConsumerState<MarkPhoneManagementPageWithAdWidget> createState() => _MarkPhoneManagementPageWithAdWidgetState();
}

class _MarkPhoneManagementPageWithAdWidgetState extends ConsumerState<MarkPhoneManagementPageWithAdWidget> {
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => [
              PopupMenuItem<String>(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.card_membership, color: Color(0xFFF5A623)),
                    const SizedBox(width: 8),
                    Text(AppLocalizations.of(context)!.exchangeVip),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildMarkedPhonesList() {
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text(
                AppLocalizations.of(context)!.noMarkedPhones,
    ...
    // 使用AdListWidget构建标记号码列表，每隔3个项目插入一个广告
    return Card(
      elevation: 4,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              AppLocalizations.of(context)!.markedPhonesList,
              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 16),
            SizedBox(
              height: 400, // 设置一个固定高度，避免无限高度问题
              child: AdListWidget<LabelPhoneEntry>(
                items: _markedPhones,
                adBuilder: () => Container(
                  height: 90,
                ...
                itemBuilder: (context, item) => _buildMarkedPhoneItem(item),
                adInterval: 3,
  ...
  Widget _buildMarkedPhoneItem(LabelPhoneEntry entry) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(entry.phoneNumber.value),
        subtitle: FutureBuilder<String>(
          future: _getLabelName(entry.labelId),
          builder: (context, snapshot) {
            return Text(snapshot.data ?? AppLocalizations.of(context)!.loading);
          },
        ),
        trailing: IconButton(
