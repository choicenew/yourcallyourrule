...
abstract class PhonelistService<T extends PhonelistEntry> {
  final Database database;
  final String tableName;

  PhonelistService(this.database, this.tableName);
  ...
  List<Map<String, dynamic>> _parseCsvData(String data) {
    ...
    return csvList.skip(1).map((parts) {
      return {
        ...
        'url': parts.length > 5 ? parts[5] : null,
      };
    }).toList();
  }
...
class SmsBlacklistService extends PhonelistService<SmsBlacklistEntry> {
  ...
  @override
  SmsBlacklistEntry fromJson(Map<String, dynamic> json) =>
      SmsBlacklistEntry.fromJson(json);

  Future<bool> isBlacklisted(String phoneNumber) => contains(phoneNumber);

  Future<List<SmsBlacklistEntry>> getSubscribedBlacklistEntries() =>
...
class SmsWhitelistService extends PhonelistService<SmsWhitelistEntry> {
  SmsWhitelistService(Database database) : super(database, 'sms_whitelisted');

  @override
  SmsWhitelistEntry fromJson(Map<String, dynamic> json) =>
      SmsWhitelistEntry.fromJson(json);

  Future<bool> isWhitelisted(String phoneNumber) => contains(phoneNumber);

  Future<List<SmsWhitelistEntry>> getSubscribedWhitelistEntries() =>
      getSubscribedEntries();
