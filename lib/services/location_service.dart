...
//分割线
  ...
  Future<LocationData?> getCallerLocationData(
      String phoneNumber, Locale locale) async {
    // 使用 PhoneNumberUtil 库获取类型
    PhoneNumberUtil phoneNumberUtil = PhoneNumberUtil.instance;
    PhoneNumber parsedPhoneNumber = phoneNumberUtil.parse(phoneNumber, null);
    ...
    // 获取来电号码的国家代码
    String? countryName = getCountryNameFromPhoneNumber(phoneNumber, locale);

    // 获取来电号码归属地
    String? region = await getRegionFromPhoneNumber(phoneNumber, locale);

    // 获取来电号码的运营商
    String? carrier = await getCarrierFromPhoneNumber(phoneNumber, locale);
