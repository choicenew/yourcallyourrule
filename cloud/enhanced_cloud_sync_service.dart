...
import 'package:notification_manager/data/services/cloud/incremental_sync_manager.dart';
import 'package:notification_manager/data/services/cloud/sync_conflict_resolver.dart';
import 'package:notification_manager/data/services/cloud/sync_progress_tracker.dart';
import 'package:notification_manager/domain/entities/device_entity.dart';
import 'package:notification_manager/domain/entities/rule_entity.dart';
import 'package:notification_manager/domain/providers/device_management_provider.dart';
import 'package:notification_manager/domain/services/cloud_sync_service.dart';
import 'package:notification_manager/domain/services/device_management_service.dart';
...
abstract class EnhancedCloudSyncService implements CloudSyncService {
  ...
  @override
  Future<bool> syncRules(List<RuleEntity> rules) async {
  ...
  Future<bool> doSyncRules(List<RuleEntity> rules);
  ...
  /// Sync notification history to the cloud with conflict resolution and progress tracking
  @override
  Future<bool> syncNotificationHistory(List<Map<String, dynamic>> notifications) async {
    if (!await isConfigured()) return false;
    
    try {
      _progressTracker.updateProgress(
        operationType: SyncOperationType.notificationSync,
        progress: 0,
        message: 'Syncing notification history to $serviceName...',
      );
      ...
      _progressTracker.updateProgress(
        operationType: SyncOperationType.notificationSync,
        progress: 50,
        message: 'Uploading notifications to $serviceName...',
      );
      
      final result = await doSyncNotificationHistory(notifications);
      
      if (result) {
        // Record sync history
        await _incrementalSyncManager.recordSyncHistory(serviceType, 'notifications', {
        ...
        // Save last sync time
        await _incrementalSyncManager.saveLastSyncTime(serviceType, 'notifications');
        ...
        _progressTracker.reportError(
          errorMessage: 'Failed to sync notification history',
          operationType: SyncOperationType.notificationSync,
        );
      }
      ...
      debugPrint('Error syncing notification history: $e');
      _progressTracker.reportError(
        errorMessage: 'Error syncing notification history: $e',
        operationType: SyncOperationType.notificationSync,
      );
      return false;
    }
  ...
  /// Implementation-specific notification history sync logic
  Future<bool> doSyncNotificationHistory(List<Map<String, dynamic>> notifications);
