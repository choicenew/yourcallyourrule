...
import 'package:notification_manager/core/config/environment.dart';
...
class OneDriveSyncService extends EnhancedCloudSyncService {
  ...
  Future<bool> doSyncNotificationHistory(List<Map<String, dynamic>> notifications) async {
    try {
      ...
      final response = await _dio.put(
        '$_driveUrl/items/$folderId:/notification_history.json:/content',
  ...
  Future<List<Map<String, dynamic>>> getNotificationHistoryFromCloud() async {
    try {
      ...
      if (response.statusCode == 200) {
        final List<dynamic> notificationsJson = jsonDecode(response.data as String);
