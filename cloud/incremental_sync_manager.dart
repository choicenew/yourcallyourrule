...
class IncrementalSyncManager {
  ...
  Future<List<RuleEntity>> syncRulesIncrementally(
    String serviceType,
    ...
    // Detect conflicts
    final conflictData = _conflictResolver.detectRuleConflicts(
      localRules,
      cloudRules,
      lastSyncHash,
    );
    ...
    final newHash = _conflictResolver.calculateRulesHash(resolvedRules);
  ...
  Future<Map<String, dynamic>> getOverallSyncStatus(String serviceType) async {
    ...
    final lastSyncNotifications = await getLastSyncTime(serviceType, 'notifications');
    ...
    final notificationsHistory = await getSyncHistory(serviceType, 'notifications');
    ...
    final lastNotificationsSync = notificationsHistory.isNotEmpty ? notificationsHistory.last : null;
    
    return {
      'lastSyncTime': {
        'rules': lastSyncRules?.toIso8601String(),
        'settings': lastSyncSettings?.toIso8601String(),
        'notifications': lastSyncNotifications?.toIso8601String(),
      },
      'lastSyncDetails': {
        'rules': lastRulesSync,
        'settings': lastSettingsSync,
        'notifications': lastNotificationsSync,
      },
      'syncHistoryCounts': {
        'rules': rulesHistory.length,
        'settings': settingsHistory.length,
        'notifications': notificationsHistory.length,
      },
    };
  }
}