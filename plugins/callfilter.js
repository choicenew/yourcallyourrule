// Call Filter Plugin - Iframe Proxy Solution
(function() {
  // --- Plugin Configuration ---
  const PLUGIN_CONFIG = {
      id: 'callfilterPlugin',
      name: 'Call Filter (iframe Proxy)',
      version: '5.5.0', // Adopted from bd.js
      description: 'Queries callfilter.app for phone number information using an iframe proxy.'
  };
   ...
   * Contains parsing logic for callfilter.app
   */
      ...
      return `
          (function() {
              ...
              function parseContent(doc) {
                  ...
                  try {
                      // --- Parsing logic from the original callfilter.js extractDataFromDOM function ---
                      const numberDiv = doc.querySelector('.number');
                      if (numberDiv) {
                          const phoneNumberDiv = numberDiv.querySelector('div > div');
                          if (phoneNumberDiv) {
                              result.phoneNumber = phoneNumberDiv.textContent.trim();
                          }
                          const labelSpan = numberDiv.querySelector('span[style="color:#000"]');
                          if (labelSpan) {
                              const labelParts = labelSpan.textContent.trim().split(',');
                              if (labelParts.length > 1) {
                                result.sourceLabel = labelParts[1].trim();
                              } else {
                                result.sourceLabel = labelParts[0].trim();
                              }
                          }
                          const provinceSpan = numberDiv.querySelector('span[id="region"]');
                          if (provinceSpan) {
                              result.province = provinceSpan.textContent.trim();
                          }
                      }
                      ...
                      if (ratingsUl) {
                          ...
                          if (advancedDiv) {
                              ...
                              if (countStrong) {
                                  result.count = parseInt(countStrong.textContent, 10);
                              }
                          }
                      }
                      ...
                      if (result.sourceLabel || result.count > 0) {
                          result.success = true;
                           // Apply manual mapping for predefinedLabel
                          let matchedLabel = manualMapping[result.sourceLabel];
                           if (matchedLabel) {
                               result.predefinedLabel = matchedLabel;
                           } else {
                               // If no manual mapping, check predefinedLabels directly
                               const predefinedMatch = predefinedLabels.find(label => label.label === result.sourceLabel);
                               if (predefinedMatch) {
                                   result.predefinedLabel = predefinedMatch.label;
                               } else if (result.sourceLabel) { // Only set unknown if there's a sourceLabel but no match
                      ...
                      console.log('[Iframe-Parser] No specific data found.');
                      return null;
              ...
              function findAndParse() {
                  ...
                  if (finalResult) {
                      ...
                      console.error('[Iframe-Parser] Failed to parse content from the page.');
  ...
  function initiateQuery(phoneNumber, requestId) {
      ...
      try {
          // Updated target URL for callfilter.app
          const targetSearchUrl = `https://callfilter.app/${phoneNumber}`;
