// tellowsar.js - Refactored to match bd.js structure
(function() {
  // --- Plugin Configuration ---
  const PLUGIN_CONFIG = {
      id: 'tellowsarPlugin', // Plugin ID, must be unique
      name: 'TellowsAR', // Plugin name
      version: '5.5.0', // Adjusted version to match bd.js structure versioning
      description: 'Queries sa.tellows.net for phone number information using an iframe proxy.', // Updated description
  };
  ...
  const manualMapping = {
    'غير معروف': 'Unknown',        // Unknown
    'رقم موثوق': 'Other',      // Trusted number - Could map to a "Safe" category
    'اليانصيب': 'Spam Likely',   // Lottery - Often spam/scam
    'شركة تحصيل ديون': 'Debt Collection',
    'إعلانات عدوانية': 'Telemarketing', // Aggressive advertising
    'دراسة': 'Survey',            // Survey
    'تحرش': 'Spam Likely',           // Harassment - Could be Fraud Scam Likely if threats
    'مصيدة تكاليف': 'Fraud Scam Likely', // Cost trap
    'مسوق عبر الهاتف': 'Telemarketing',
    'بينغ نداء': 'Spam Likely',     // Ping Call
    'احتيال النص': 'Fraud Scam Likely',  // Text fraud
    'مكالمة احتيالية': 'Fraud Scam Likely', // Keep mapping, used from name.
    'اتصالات بعد منتصف الليل': 'Spam Likely', // Contextual, keep it
    'مزعج' : 'Spam Likely', // Keep this mapping
    'مسابقة': 'Other',   // Keep mapping
    'تهديد': 'Fraud Scam Likely', // Keep mapping
     'غير محدد': 'Unknown', // Keep mapping
    'Unknown': 'Unknown',
  ...
  function initiateQuery(phoneNumber, requestId) {
      ...
      try {
          const targetSearchUrl = `https://sa.tellows.net/num/${encodeURIComponent(phoneNumber)}?lang=en`; // Updated URL
          const config = window.plugin[PLUGIN_CONFIG.id].config || {};
          const userAgent = config.userAgent || 'Mozilla/5.0 (Linux; arm_64; Android 14; SM-S711B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 YaBrowser/24.12.4.199.00 SA/3 Mobile Safari/537.36';
